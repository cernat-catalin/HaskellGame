...
import           GMessages.Network.ClientServer ( Message(..)
                                                , ConnectionMessage(..)
                                                , ServiceMessage(..)
                                                )
import qualified GMessages.Network.ServerClient
                                               as SC
...
initialSetup connHandle = do
  ...
  _        <- sendMessage
    connHandle
    (ServiceMessage 0 $ ConnectionMessage $ ConnectionRequest settings)
...
gatherSettings = do
  ...
  name <- getLine
  team <- getTeam
  PlayerSettings name team <$> getVehicle

gatherSettingsReset :: IO PlayerSettingsReset
gatherSettingsReset = do
  team <- getTeam
  PlayerSettingsReset team <$> getVehicle
...
meshMapSetup objectsMap = do
  ...
  let meshMap =
        HMap.insert "background" background'
          ...
          . HMap.insert "bullet3Blue" bullet3BlueMesh'
          $ HMap.empty
  return meshMap
...
meshObjectsMapSetup = do
  ...
  let meshObjectsMap =
        HMap.insert "background" background'
          ...
          . HMap.insert "squareShallow" squareShallow'
          $ HMap.empty
